...
@elseif($metrics['total_trades'] === 0)
    <div class="flex flex-col items-center justify-center rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 py-20 text-center">
        ...
        <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ __('labels.register_trade_to_see_metrics') }}</p>
    </div>
@else
    @php
        $m = $metrics;
    ...
    {{-- ═══ ANÁLISIS IA ═══════════════════════════════════════════ --}}
    <div class="mb-4 rounded-xl border border-indigo-100 bg-gradient-to-r from-indigo-50/70 to-white p-4 dark:border-indigo-500/30 dark:from-indigo-500/10 dark:to-gray-800">
        <div class="flex items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-lg dark:bg-indigo-500/20">
                    🤖
                </div>
                <div>
                    <p class="text-sm font-bold text-gray-900 dark:text-gray-100">{{ __('labels.bt_ai_title') }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('labels.bt_ai_subtitle') }}</p>
                </div>
            </div>
        ...
        @if ($selectedStrategy->ai_analysis)
            <div class="prose prose-sm mt-4 max-w-none rounded-lg border border-indigo-100/60 bg-white dark:bg-gray-800 p-4 text-sm leading-relaxed text-gray-700 dark:text-gray-200">{!! \Illuminate\Support\Str::of($selectedStrategy->ai_analysis)->stripTags()->markdown(['html_input' => 'strip']) !!}</div>
        @endif
    </div>
    ...
    <div class="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
            <p class="mb-1 text-xs text-gray-400 dark:text-gray-500">{{ __('labels.trades') }}</p>
            <p class="text-2xl font-bold tabular-nums text-gray-900 dark:text-gray-100">{{ $m['total_trades'] }}</p>
        ...
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
            <p class="mb-1 text-xs text-gray-400 dark:text-gray-500">{{ __('labels.win_rate') }}</p>
        ...
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
            <p class="mb-1 text-xs text-gray-400 dark:text-gray-500">{{ __('labels.profit_factor') }}</p>
            @php
                $pf = $m['profit_factor'];
            ...
            <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ $pfLabel }}</p>
        </div>

        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
            <p class="mb-1 text-xs text-gray-400 dark:text-gray-500">{{ __('labels.avg_r') }}</p>
            <p class="{{ $m['avg_r'] >= 0 ? 'text-emerald-600' : 'text-red-500' }} text-2xl font-bold tabular-nums">
                {{ $m['avg_r'] > 0 ? '+' : '' }}{{ $m['avg_r'] }}R
            </p>
            <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">Expectancy: {{ $m['expectancy'] > 0 ? '+' : '' }}{{ $m['expectancy'] }}R</p>
        </div>

        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
            <p class="mb-1 text-xs text-gray-400 dark:text-gray-500">{{ __('labels.avg_profit_trade') }}</p>
            <p class="text-xl font-bold tabular-nums text-emerald-600">
                {{ $m['avg_win'] > 0 ? '+' : '' }}{{ $m['avg_win'] }}{{ $unit }}
            </p>
            <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">Mayor: <span class="font-medium text-emerald-500">+{{ $m['biggest_win'] }}{{ $unit }}</span></p>
        </div>

        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
            <p class="mb-1 text-xs text-gray-400 dark:text-gray-500">{{ __('labels.avg_losser_trade') }}</p>
            <p class="text-xl font-bold tabular-nums text-red-500">{{ $m['avg_loss'] }}{{ $unit }}</p>
            <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">Mayor: <span class="font-medium text-red-400">{{ $m['biggest_loss'] }}{{ $unit }}</span></p>
        </div>

        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
            <p class="mb-1 text-xs text-gray-400 dark:text-gray-500">{{ __('labels.max_drawdown') }}</p>
            <p class="text-xl font-bold tabular-nums text-red-500">{{ $dd['percent'] }}%</p>
            <p class="mt-1 text-xs tabular-nums text-gray-400 dark:text-gray-500">{{ $dd['amount'] }}{{ $unit }}</p>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
            ...
            @else
                ...
                <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ __('labels.annualized_return') }}</p>
            @endif
        </div>
    ...
    <div class="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
            <h3 class="mb-4 text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('labels.efficiency_of_trader') }}</h3>
            ...
            <div class="mb-4 flex items-center gap-4">
                <div class="relative h-16 w-16 shrink-0">
                    ...
                    <span class="absolute inset-0 flex items-center justify-center text-sm font-bold text-gray-900 dark:text-gray-100">
                        {{ $eff['score'] }}
                    </span>
                </div>
                <div>
                    <p class="text-{{ $effLabel[1] }}-600 text-base font-semibold">{{ $effLabel[0] }}</p>
                    <p class="text-xs text-gray-400 dark:text-gray-500">{{ __('labels.score_over_100') }}</p>
                </div>
            </div>
            ...
            <div class="space-y-2.5">
                <div>
                    <div class="mb-1 flex justify-between text-xs">
                        <span class="text-gray-500 dark:text-gray-400">{{ __('labels.discipline_rules') }}</span>
                        <span class="font-medium text-gray-700 dark:text-gray-200">{{ $eff['rules_followed_pct'] }}%</span>
                    </div>
                ...
                <div>
                    <div class="mb-1 flex justify-between text-xs">
                        <span class="text-gray-500 dark:text-gray-400">{{ __('labels.quality_setups') }}</span>
                        <span class="font-medium text-gray-700 dark:text-gray-200">{{ $eff['high_quality_pct'] }}%</span>
                    </div>
                    <div class="h-1.5 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700">
                ...
                <div>
                    <div class="mb-1 flex justify-between text-xs">
                        ...
                        <span class="font-medium text-gray-700 dark:text-gray-200">{{ $m['win_rate'] }}%</span>
                    </div>
                    <div class="h-1.5 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700">
                ...
                <div class="flex justify-between border-t border-gray-100 dark:border-gray-700 pt-1 text-xs">
                    <span class="text-gray-400 dark:text-gray-500">{{ __('labels.avg_rr_real') }}</span>
                    <span class="font-semibold text-gray-700 dark:text-gray-200">{{ $eff['avg_rr'] }}R</span>
                </div>
            </div>
        </div>

        {{-- Curva de capital --}}
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5 sm:col-span-2">
            <div class="mb-4 flex items-center justify-between">
                <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('labels.equity_curve_title') }}</h3>
    ...
    <div class="mb-4 overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800"
         x-data="{
             selectedDate: null,
        ...
        <div class="grid grid-cols-12 divide-x divide-gray-100 dark:divide-gray-700"
            ...
            <div class="col-span-6 p-5"
                ...
                <div class="mb-5 flex items-center justify-between">
                    <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('labels.ops_calendar') }}</h3>
                    <div class="flex items-center gap-2">
                        <button class="flex h-7 w-7 items-center justify-center rounded-lg border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 transition-colors hover:bg-gray-50 dark:hover:bg-gray-900 hover:text-gray-700 dark:hover:text-gray-200"
                                @click="prevMonth()">
                        ...
                        <span class="w-36 text-center text-sm font-semibold text-gray-700 dark:text-gray-200"
                ...
                <div class="mb-2 grid grid-cols-7">
                    <template x-for="d in ['Lun','Mar','Mié','Jue','Vie','Sáb','Dom']">
                        <div class="py-1 text-center text-[10px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500"
                             x-text="d"></div>
                    </template>
                </div>
                ...
                <div class="grid grid-cols-7 gap-1.5">
                    ...
                    <template x-for="i in firstDayOfMonth(currentMonth, currentYear)"
                              :key="'empty-' + i">
                        <div class="h-16 rounded-xl border border-dashed border-gray-100 dark:border-gray-700 bg-gray-50/40 dark:bg-gray-900/40"></div>
                    </template>
                        ...
                        <div>
                            ...
                            <template x-if="getDayData(day)">
                                <div class="relative flex h-16 cursor-pointer select-none flex-col justify-between rounded-xl border p-2 transition-all duration-150"
                                    ...
                                    <div class="flex items-end justify-between">
                                        ...
                                        <span class="text-[9px] leading-none text-gray-400 dark:text-gray-500"
                            ...
                            <template x-if="!getDayData(day)">
                                <div class="relative flex h-16 flex-col justify-between rounded-xl border p-2 transition-colors"
                                     :class="{
                                         'bg-gray-50/60 dark:bg-gray-900/60 border-gray-100 dark:border-gray-700': isWeekend(day),
                                         'bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700': !isWeekend(day),
                                         'ring-1 ring-blue-300 border-blue-200': isToday(day)
                                     }">

                                    <span class="text-sm leading-none"
                                          :class="{
                                              'text-gray-300 dark:text-gray-600 font-normal': isWeekend(day),
                                              'text-gray-400 dark:text-gray-500 font-medium': !isWeekend(day) && !isToday(day),
                ...
                {{-- Leyenda --}}
                <div class="mt-4 flex items-center gap-4 border-t border-gray-100 dark:border-gray-700 pt-3">
                    <div class="flex items-center gap-1.5">
                        <div class="h-3 w-3 rounded-sm border border-emerald-200 bg-emerald-100"></div>
                        <span class="text-[10px] text-gray-400 dark:text-gray-500">{{ __('labels.positive_day') }}</span>
                    </div>
                    <div class="flex items-center gap-1.5">
                        <div class="h-3 w-3 rounded-sm border border-red-200 bg-red-100"></div>
                        <span class="text-[10px] text-gray-400 dark:text-gray-500">{{ __('labels.negative_day') }}</span>
                    </div>
                    <div class="flex items-center gap-1.5">
                        <div class="h-1 w-8 rounded-full bg-gradient-to-r from-red-400 to-emerald-400"></div>
                        <span class="text-[10px] text-gray-400 dark:text-gray-500">{{ __('labels.bar_winrate_legend') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-span-6 flex flex-col p-5">
                ...
                <template x-if="!selectedDate">
                    <div class="flex flex-1 flex-col items-center justify-center text-center">
                        <svg class="mb-2 h-8 w-8 text-gray-200 dark:text-gray-600"
                             xmlns="http://www.w3.org/2000/svg"
                             fill="none"
                             viewBox="0 0 24 24"
                             stroke="currentColor"
                        ...
                        <p class="text-sm text-gray-400 dark:text-gray-500">{{ __('labels.select_calendar_day') }}</p>
                        <p class="mt-1 text-xs text-gray-300 dark:text-gray-600">{{ __('labels.days_with_ops_colored') }}</p>
                    </div>
                </template>
                    ...
                    <div class="flex h-full flex-col">
                        ...
                        <div class="mb-4 flex items-center justify-between">
                            <div>
                                ...
                                <div class="mt-0.5 flex items-center gap-3">
                                    ...
                                    <span class="text-xs text-gray-400 dark:text-gray-500">
                                        WR: <span class="font-medium text-gray-600 dark:text-gray-300"
                                              x-text="selectedDayData?.winrate + '%'"></span>
                                    </span>
                                </div>
                            </div>
                            <button class="flex h-6 w-6 items-center justify-center rounded-full text-gray-400 dark:text-gray-500 transition-colors hover:bg-gray-100 dark:hover:bg-gray-700"
                        ...
                        <div class="flex-1 space-y-2 overflow-y-auto"
                            ...
                            <template x-if="selectedTrades.length === 0">
                                <p class="py-4 text-center text-xs text-gray-400 dark:text-gray-500">{{ __('labels.no_operations_registered') }}</p>
                            </template>
                            ...
                            <template x-for="trade in selectedTrades"
                                ...
                                <div class="mx-1 flex cursor-pointer items-center justify-between rounded-lg border p-3 transition-colors hover:ring-2 hover:ring-blue-300"
                                     :class="trade.pnl_r > 0 ? 'bg-emerald-50 border-emerald-100' : (trade.pnl_r < 0 ? 'bg-red-50 border-red-100' : 'bg-gray-50 dark:bg-gray-900 border-gray-100 dark:border-gray-700')"
             ...
             screenshot:     trade.screenshot ?? null
                                    ...
                                    <div class="flex items-center gap-3">
                                        {{-- Sesión --}}
                                        <span class="text-xs capitalize text-gray-400 dark:text-gray-500"
                                        ...
                                        <span class="w-14 text-right text-sm font-bold tabular-nums"
                                              :class="trade.pnl_r > 0 ? 'text-emerald-600' : (trade.pnl_r < 0 ? 'text-red-500' : 'text-gray-400 dark:text-gray-500')"
    ...
    <div class="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        {{-- Win Rate por día --}}
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
            <h3 class="mb-4 text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('labels.winrate_by_day') }}</h3>
            <div class="space-y-2">
                @foreach ($m['daily_winrate'] as $day)
                    @if ($day['total'] > 0)
                        <div class="flex items-center gap-3">
                            <span class="w-8 text-xs font-medium text-gray-500 dark:text-gray-400">{{ $day['day'] }}</span>
                            <div class="h-5 flex-1 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700">
                            ...
                            <span class="w-12 text-right text-xs tabular-nums text-gray-400 dark:text-gray-500">{{ $day['total'] }} ops</span>
                        </div>
                    @endif
                @endforeach

                @if (collect($m['daily_winrate'])->sum('total') === 0)
                    <p class="py-4 text-center text-sm text-gray-400 dark:text-gray-500">{{ __('labels.no_data') }}</p>
                @endif
            </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
            <h3 class="mb-4 text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('labels.performance_by_session') }}</h3>
            @if (!empty($m['pnl_by_session']['labels']))
                <div class="space-y-3">
                    @foreach ($m['pnl_by_session']['labels'] as $i => $label)
                        ...
                        @endphp
                        <div class="flex items-center justify-between rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-3">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs text-gray-400 dark:text-gray-500">({{ $sessionN }})</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="text-right">
                                    <p class="text-xs text-gray-400 dark:text-gray-500">WR</p>
                                    <p class="{{ $sessionWr >= 50 ? 'text-emerald-600' : 'text-red-500' }} text-sm font-bold">{{ $sessionWr }}%</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-xs text-gray-400 dark:text-gray-500">PnL</p>
    ...
    <div class="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
            <h3 class="mb-4 text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('labels.r_distribution') }}</h3>
            <div id="chart-r-dist"
                 style="min-height:200px"></div>
        </div>

        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
            <h3 class="mb-1 text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('labels.rules_impact_title') }}</h3>
            <p class="mb-4 text-xs text-gray-400 dark:text-gray-500">{{ __('labels.followed_rules_vs_not') }}</p>
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between rounded-lg border border-emerald-100 bg-emerald-50 p-3">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-200">{{ __('labels.followed_rules') }}</span>
                            <span class="text-xs text-gray-400 dark:text-gray-500">({{ $ri['followed']['count'] }})</span>
                        </div>
                        <div class="flex items-center gap-4 text-right">
                            ...
                            <div>
                                <p class="text-xs text-gray-400 dark:text-gray-500">Avg R</p>
                    ...
                    <div class="flex items-center justify-between rounded-lg border border-red-100 bg-red-50 p-3">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-200">{{ __('labels.not_followed_rules') }}</span>
                            <span class="text-xs text-gray-400 dark:text-gray-500">({{ $ri['not_followed']['count'] }})</span>
                        </div>
                        <div class="flex items-center gap-4 text-right">
                            <div>
                                <p class="text-xs text-gray-400 dark:text-gray-500">WR</p>
            ...
            @else
                <p class="py-6 text-center text-sm text-gray-400 dark:text-gray-500">{{ __('labels.no_enough_data') }}</p>
            @endif
        </div>
    ...
    <div class="mb-4 grid grid-cols-1 gap-4 xl:grid-cols-2">
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
            <div class="border-b border-gray-100 dark:border-gray-700 px-5 py-4">
                <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('labels.rating_vs_performance') }}</h3>
                <p class="mt-0.5 text-xs text-gray-400 dark:text-gray-500">{{ __('labels.best_setups_perform_more') }}</p>
            </div>
            ...
            @if ($ratingRows->isEmpty())
                <div class="px-5 py-10 text-center text-sm text-gray-400 dark:text-gray-500">{{ __('labels.no_enough_data_short') }}</div>
            @else
                <div class="p-4">
                    <div class="space-y-2">
                        @foreach ($ratingRows->sortByDesc('rating') as $row)
                            @php
                                ...
                                [$badge, $badgeColor] = match (true) {
                                    ...
                                    $row['avg_r'] >= 0 => [__('labels.label_neutral'), 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400'],
                                    $row['avg_r'] >= -0.5 => [__('labels.label_weak'), 'bg-amber-100 text-amber-700'],
                                    default => [__('labels.label_avoid'), 'bg-red-100 text-red-600'],
                                };
                            @endphp
                            <div class="{{ $isPositive ? 'border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900' : 'border-red-50 bg-red-50/40' }} relative overflow-hidden rounded-lg border px-4 py-3">
                                ...
                                <div class="relative flex items-center gap-4">
                                    <div class="flex w-36 shrink-0 flex-col gap-1">
                                        <div class="flex items-center gap-0.5">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <svg class="{{ $i <= $row['rating'] ? 'text-amber-400' : 'text-gray-200 dark:text-gray-600' }} h-3.5 w-3.5"
                                                     fill="currentColor"
                                                     viewBox="0 0 20 20">
                                                    <path
                                    ...
                                    <div class="flex flex-1 items-center justify-between gap-2">
                                        <div class="text-center">
                                            ...
                                            <p class="text-[10px] text-gray-400 dark:text-gray-500">avg R</p>
                                        </div>
                                        <div class="h-8 w-px bg-gray-200 dark:bg-gray-700"></div>
                                        <div class="text-center">
                                            <p class="{{ $row['wr'] >= 50 ? 'text-emerald-600' : 'text-red-500' }} text-base font-bold tabular-nums">{{ $row['wr'] }}%</p>
                                            <p class="text-[10px] text-gray-400 dark:text-gray-500">winrate</p>
                                        </div>
                                        <div class="h-8 w-px bg-gray-200 dark:bg-gray-700"></div>
                                        <div class="text-center">
                                            <p class="text-base font-bold tabular-nums text-gray-700 dark:text-gray-200">{{ $row['count'] }}</p>
                                            <p class="text-[10px] text-gray-400 dark:text-gray-500">trades</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
            <div class="border-b border-gray-100 dark:border-gray-700 px-5 py-4">
                <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('labels.confluence_analysis_title') }}</h3>
                <p class="mt-0.5 text-xs text-gray-400 dark:text-gray-500">{{ __('labels.technical_factors_correlation') }}</p>
            </div>
            ...
            @if (empty($confluences))
                <div class="flex flex-col items-center justify-center px-5 py-10 text-center">
                    <p class="text-sm text-gray-400 dark:text-gray-500">{{ __('labels.no_confluences_registered') }}</p>
                    <p class="mt-1 text-xs text-gray-300 dark:text-gray-600">{{ __('labels.add_confluences_to_trades') }}</p>
                </div>
            @else
                ...
                <div class="p-4">
                    <div class="space-y-2">
                        @foreach ($confluences as $c)
                            ...
                            @endphp
                            <div class="{{ $cPositive ? 'border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900' : 'border-red-50 bg-red-50/40' }} relative overflow-hidden rounded-lg border px-4 py-3">
                                ...
                                <div class="relative flex items-center justify-between gap-4">
                                    ...
                                    {{-- Nombre --}}
                                    <span class="w-24 shrink-0 truncate text-sm font-semibold text-gray-700 dark:text-gray-200">
                                        {{ $c['name'] }}
                                    </span>
                                    ...
                                    <div class="flex flex-1 items-center justify-end gap-4">
                                        ...
                                        <div class="text-center">
                                            ...
                                            <p class="text-[10px] text-gray-400 dark:text-gray-500">WR</p>
                                        </div>
                                        <div class="h-8 w-px bg-gray-200 dark:bg-gray-700"></div>
                                        <div class="text-center">
                                            <p class="text-sm font-bold tabular-nums text-gray-700 dark:text-gray-200">{{ $c['count'] }}</p>
                                            <p class="text-[10px] text-gray-400 dark:text-gray-500">trades</p>
                                        </div>
                                        <div class="h-8 w-px bg-gray-200 dark:bg-gray-700"></div>
                                        <div class="text-center">
                                            ...
                                            <p class="text-[10px] text-gray-400 dark:text-gray-500">total R</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
